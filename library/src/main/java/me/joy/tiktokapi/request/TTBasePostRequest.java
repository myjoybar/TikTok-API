...
public abstract class TTBasePostRequest<R extends TTBaseResponseData> extends TTBaseRequest<R> {

  private static final String TAG = "TTBasePostRequest";
  private BaseRequestBuilder requestBuilder = getBaseRequestBuilder();

  @Override
  protected void execute() {
    // BaseRequestBuilder baseRequestBuilder = getBaseRequestBuilder();
    requestBuilder.addHeaders(APIConfig.getHeaders())
  ...
  public enum TypePost {
    POST_STRING, POST_FORM, POST_FILE
  }

  private BaseRequestBuilder getBaseRequestBuilder() {
    if (getTypePost() == TypePost.POST_FORM) {
      return OkHttpManager.getInstance().postForm(getRequestUrl());
    } else if (getTypePost() == TypePost.POST_FILE) {
      return OkHttpManager.getInstance().postFile(getRequestUrl());
    }
    return OkHttpManager.getInstance().postString(getRequestUrl());
  }
